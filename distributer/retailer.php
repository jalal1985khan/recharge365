...
if(isset($_POST['submitretailer'] )){
   ...
   $query = "SELECT * FROM `distributer` WHERE `MOBILE` = '$mobile' ";
    ...
    if(mysqli_num_rows($run) < 1 && $query2 != 1 && $query3 != 1) {
            ...
            $q = "SELECT * FROM `distributer` WHERE ID = '$ds_id'";
            $User = mysqli_fetch_array(mysqli_query($con , $q));
            $ms_id = $User['MS_ID'];
            $date = date("Y-m-d");
        	$query2 = "INSERT INTO `retailer`(`OWNER` , `MS_ID`,`DISTRIBUTER`, `FNAME`,`MOBILE`, `EMAIL`, `SMSBAL`, `RCBAL`, `DMRBAL`, `COMM_PACK`, `CUTTOFFAMOUNT`, `STATUS`, `APIACCESS`, `ADDRESS`, `CITY`,`STATE`,  `PASSWORD` , `IMAGE` , `REGDATE`) 
        	VALUES('DISTRIBUTER' , '$ms_id' , '$ds_id' , '$firstname' , '$mobile' , '$email', '0', '0', '0',  '0', '10', 'Activate', '', '$address' , '$city' , '$state' , '$pass_hash' ,'default.jpeg' ,'$date') ";
                                                ...
                                                <div class="table-responsive dt-responsive">
                                                    <table id="custm-tool-ele"
                                                        ...
                                                        <tbody>
                                                            <?php
                                                    $ds_id = $_SESSION["ds_id"];
                                                    $res = $con->query("SELECT * FROM retailer WHERE `DISTRIBUTER`='$ds_id'  order by ID asc");
                                                    if($res->num_rows > 0){
