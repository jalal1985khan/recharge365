<?php
session_start();
if(!isset($_SESSION['ds_id']) || $_SESSION['ds_id']==="0"){
...
require("../includes/config.php");
   ...
   if(isset($_POST['addpackage']))
   {
     ...
     $status = $_POST['status'];
     $ad_id = $_SESSION['ds_id'];
     
    $query = "INSERT INTO `commPackage`(`OWNER` , `OWNER_ID` ,`USERTYPE`, `PACKNAME`, `COMM` , `STATUS`) VALUES ('DISTRIBUTER' , '$ad_id' , '$usertype' , '$packname' , '0' , '$status') ";
   ...
   if($run) {
             $operator = $con->query("select * from switchOperator ");
                while($all_op = $operator->fetch_assoc()){
                    ...
                    $pack = $con->query("select * from commPackage where  OWNER='DISTRIBUTER' and OWNER_ID='$ad_id' and PACKNAME='$packname'")->fetch_assoc();
                    $pack_id  = $pack['ID'];
                                                    ...
                                                    <table id="custm-tool-ele"
                                                        ...
                                                        <tbody>
                                                           ...
                                                           $ms_id = $_SESSION['ds_id'];
                                                            $res = $con->query("SELECT * FROM commPackage where OWNER='DISTRIBUTER' and OWNER_ID='$ms_id' order by ID asc");
                                                            if($res->num_rows > 0){
