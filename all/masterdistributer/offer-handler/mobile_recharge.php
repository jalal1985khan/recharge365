     ...
     curl_close ($ch);
      ...
      foreach($status as $key => $value) {
            echo '<div class="col-6">
                        <p class="para">Balance</p>
                        <p class="para">customerName</p>
                        <p class="para">NextRechargeDate</p>
                        <p class="para">status</p>
                        <p class="para">MonthlyRecharge</p>
                        <p class="para">planname</p>
                    </div>
                <div class="col-6">
                      <p class="para">'.$value->Balance.'</p>
                        <p class="para">'.$value->customerName.'</p>
                        <p class="para">'.$value->NextRechargeDate.'</p>
                        <p class="para">'.$value->status.'</p>
                        <p class="para">'.$value->MonthlyRecharge.'</p>
                        <p class="para">'.$value->planname.'</p>
                </div>';
          }
}
...
function apkel($op , $num){
    ...
    //   print_r($result);
           foreach($status as $key => $value) {
            echo '<div class="col-6">
                    <p class="para">CustomerName</p>
                    <p class="para">BillNumber</p>
                    <p class="para">Billdate</p>
                    <p class="para">Billamount</p>
                    <p class="para">Duedate</p>
                </div>
                <div class="col-6">
                    <p class="para">'.$value->CustomerName.'</p>
                    <p class="para">'.$value->BillNumber.'</p>
                    <p class="para">'.$value->Billdate.'</p>
                    <p class="para">'.$value->Billamount.'</p>
                    <p class="para">'.$value->Duedate.'</p>
                </div>';
          }
}
...
if(isset($_POST['dth_code'])){
    ...
    // echo "dth";
   $op = $con->query("select * from switchOperator where LONGCODE='$op_code'")->fetch_assoc();
	//echo "select * from switchOperator where PRODUCTNAME='$op_code'";
    $r_name = $op['roffer'];
    $number = $_POST['number'];
	// echo $r_name;
    // dthoffer($r_name , $number);
	 dthplan($r_name , $number);
}
...
if(isset($_POST['apk_dth'])){
        $op_code = $_POST['apk_dth'];
    // echo "dth";
   $op = $con->query("select * from switchOperator where PRODUCTNAME='$op_code'")->fetch_assoc();
    $r_name = $op['roffer'];
    $number = $_POST['number'];
    apkdth($r_name , $number);
}
if(isset($_POST['apk_el'])){
        $op_code = $_POST['apk_el'];
    // echo "dth";
   $op = $con->query("select * from switchOperator where PRODUCTNAME='$op_code'")->fetch_assoc();
    $r_name = $op['roffer'];
    $number = $_POST['number'];
