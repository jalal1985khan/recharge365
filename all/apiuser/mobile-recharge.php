...
if(isset($_POST['submit_rech'])){
    ...
    if($amount=='' || $mobile=='' || strlen($mobile)!==10 || $amount < 10){
             ...
             $q = $con->query("SELECT * FROM Api_users where ID='$ap_id'")->fetch_assoc();
             ...
             $api_key = $q['API_KEY'];
                 if($amount < $ms_rcbal){
                     if(($ms_rcbal - $amount) < $ms_ctof){
                       ...
                       $op = $con->query("select * from switchOperator where LONGCODE='$operator'")->fetch_assoc();

                    $op_name = $op['PRODUCTNAME'];
                    $serchApi = $op['APICOMPANY'];
                    $backup_api = $op['BACKUP_API'];
                    // echo $serchApi;
                    api_recharge($serchApi , $mobile , $operator , $amount , $api_key ,$backup_api , $op_name);
                  }
             }else{
