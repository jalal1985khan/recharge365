...
if (isset($_POST['masterdistributer'])) {
    $ms_id = $_POST['ms_id'];
    $_SESSION["ms_id"] = $_POST['ms_id'];
    $sms_id = $_SESSION["ms_id"];
    $amount = $_POST['amount'];
    $type = $_POST['type'];
    $remark = $_POST['remark'];
    ...
    if ($type == "add") {
        ...
        $user_rc = $user['RCBAL'];
        $total_amount = round($user_rc + $amount);
        $sms_name = $_SESSION["NAME"] = $user['NAME'];
        ...
        if($amount > $admin_rc){
            ...
            $con->query("UPDATE admin SET RCBAL='$ad_am' WHERE ID='$admin_id'");
...
if (isset($_POST['distributer'])) {
    ...
    $remark = $_POST['remark'];
    $admin_id = $_SESSION["status"];
    $sms_id = $_SESSION["ms_id"] = $_POST['ds_id'];
    if ($type == "add") {
...
if (isset($_POST['retailer'])) {
    ...
    $remark = $_POST['remark'];
    $admin_id = $_SESSION["status"];
    $sms_id = $_SESSION["ms_id"] = $_POST['rt_id'];
    if ($type == "add") {
        $user = $con->query("select * from retailer where ID='$ms_id'")->fetch_assoc();
        $user_rc = $user['RCBAL'];
        $total_amount = $user_rc + $amount;
        $sms_name = $_SESSION["NAME"] = $user['FNAME'];
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Recharge Fund <?php echo  $admin_rc;?></h1>
                </div>
                <div class="col-sm-6">
                                            ...
                                            <div class="input-group col-sm-6">
                                                <select id="hello-single" name="ms_id" class="form-control stock">
                                                    ...
                                                    while ($row = $q->fetch_assoc()) {
                                                        echo '<option value="' . $row['ID'] . '">' . strtoupper($row['NAME']) .' ('. round($row['RCBAL']).') </option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                                        ...
                                        <div class="form-group row mt-3">
                                            <div class="input-group col-sm-6">
                                                <select id="hello-single" name="rt_id" class="form-control stock">
                                                    ...
                                                    while ($row = $q->fetch_assoc()) {
                                                        echo '<option value="' . $row['ID'] . '">' . strtoupper($row['FNAME']) .' ('. round($row['RCBAL']).')</option>';
                                                    }
                                                    ?>
                                                </select>
                                            </div>
